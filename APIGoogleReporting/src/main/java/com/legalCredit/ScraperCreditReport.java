...
public class ScraperCreditReport implements HttpFunction {
	...
	private String credit_report_id;
	private String sf_client_id;
	...
	public String scrapingPDF(String Json,HttpResponse response) {
		...
		try {
			...
			credit_report_id = payLoad.getString("credit_report_id");
			sf_client_id = payLoad.getString("sf_client_id");
			assigned_agent =  payLoad.getString("assigned_agent");
			bureau_id =  payLoad.getString("bureau_id");
			byte[] decoded = Base64.getDecoder().decode(payLoad.getString("file"));
	...
	private String generarJSON(Reporte reporte,List<CuentaReporte> cuentasReporte, 
		...
		if (reporte.getAddress()!=null && (length = reporte.getAddress().length) > 0) {
			...
			length--;
			last_reported_employment = reporte.getAddress()[length];

			length--;
			if (length > 0)
				previous_reported_employment = reporte.getAddress()[length];
		}
		...
		jsonReporte.put("credit_report_id",credit_report_id) ;
		jsonReporte.put("sf_client_id", sf_client_id);
		jsonReporte.put("assigned_agent", assigned_agent)  ;
		jsonReporte.put("bureau_id",bureau_id) ;
		jsonReporte.put("credit_file_date",reporte.getDateofreport());
		...
		for (CuentaReporte cuentaReporte : cuentasReporte) {
			...
			jsonCuentaReporte.put("account_number",cuentaReporte.getAccountNumber());
		...
		jsonReporte.put("credit_reports_items",cuentasReporteArray);
		...
		if (cuentaInquieries != null) {
			...
			for (Inquiery inquiery : cuentaInquieries) {
				...
				jsonCuentaInquiries.put("account_nName",inquiery.getAccountName()); 
		...
		if (publicAccounts != null) {
			...
			for (PublicAccount publicAccount : publicAccounts) {
				...
				publicAccountReporte.put("plain_tiff",publicAccount.getPlaintiff().replaceAll("( )+", " "));
